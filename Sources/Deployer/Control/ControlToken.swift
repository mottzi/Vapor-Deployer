...
struct ControlToken {
    ...
    /// Returns `nil` if the file cannot be read or created — the control route is then not mounted and CLI callers
    /// see 404 (treated as "refuse" rather than "proceed", per ADR 0005).
    static func loadOrGenerate(installDirectory: URL) -> ControlToken? {
        ...
        if fileManager.fileExists(atPath: fileURL.path) {
            ...
            return ControlToken(value: trimmed)
        }
        ...
        return ControlToken(value: token)
    }
