    ...
    func run(using context: CommandContext, signature: Signature) async throws {
        ...
        let response = try await context.application.client.post(uri) {
            $0.headers.add(
                ...
                value: Deployer.Variables.DEPLOY_SECRET.value
            )
        }
...
extension DeployCommand {
    ...
    func useRoute(config: DeployerConfiguration) {
        ...
        app.post("\(config.panelRoute)/deploy") { request async throws -> String in

            guard let providedSecret = request.headers.first(name: "X-Deploy-Secret"),
                  let expectedSecret = Environment.get(Deployer.Variables.DEPLOY_SECRET.rawValue)
