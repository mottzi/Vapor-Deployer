...
struct DeployerctlInstallStep: SetupStep {
    ...
    func run(context: SetupContext, console: any Console) async throws {
        ...
        try await SetupFileSystem.writeFile(try DeployerctlScriptTemplate.wrapperConfig(context: context), to: paths.deployerctlConfig)
        try await SetupFileSystem.writeFile(DeployerctlScriptTemplate.wrapperScript(), to: paths.deployerctlBinary, mode: "0755")
