        ...
        if await userExists(context.serviceUser) {
            ...
            if FileManager.default.fileExists(atPath: "\(paths.installDirectory)/.git") {
                if context.buildFromSource {
                    let origin = try await UserShell.runAsServiceUser(context, ["git", "-C", paths.installDirectory, "remote", "get-url", "origin"]).trimmed
                    ...
                    let dirty = try await UserShell.runAsServiceUser(context, ["git", "-C", paths.installDirectory, "status", "--porcelain", "--untracked-files=no"]).trimmed
            ...
            if FileManager.default.fileExists(atPath: "\(paths.appDirectory)/.git") {
                let origin = try await UserShell.runAsServiceUser(context, ["git", "-C", paths.appDirectory, "remote", "get-url", "origin"]).trimmed
                ...
                let dirty = try await UserShell.runAsServiceUser(context, ["git", "-C", paths.appDirectory, "status", "--porcelain", "--untracked-files=no"]).trimmed
                guard dirty.isEmpty else {
