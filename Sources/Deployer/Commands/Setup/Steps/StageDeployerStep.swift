...
struct StageDeployerStep: SetupStep {
    ...
    func run() async throws {
        ...
        try await rescueDatabaseFromModeSwitchBackup()
    }
...
extension StageDeployerStep {
    ...
    /// When Preflight renames the prior install to `installDirectory.bak` (build mode change), the new tree has no database yet—copy the old `deployer.db` (and SQLite sidecars) before later steps and cleanup remove the backup.
    private func rescueDatabaseFromModeSwitchBackup() async throws {
        let backupRoot = paths.installDirectory + ".bak"
        let backupDB = "\(backupRoot)/deployer.db"
        let activeDB = "\(paths.installDirectory)/deployer.db"
        guard FileManager.default.fileExists(atPath: backupDB),
              !FileManager.default.fileExists(atPath: activeDB)
        else { return }
        
        console.print("Restoring deployer.db from the previous installation (build mode change)...")
        var copiedPaths: [String] = []
        for name in ["deployer.db", "deployer.db-wal", "deployer.db-shm"] {
            let source = "\(backupRoot)/\(name)"
            guard FileManager.default.fileExists(atPath: source) else { continue }
            let destination = "\(paths.installDirectory)/\(name)"
            try FileManager.default.copyItem(atPath: source, toPath: destination)
            copiedPaths.append(destination)
        }
        if !copiedPaths.isEmpty {
            try await Shell.runThrowing("chown", ["\(context.serviceUser):\(context.serviceUser)"] + copiedPaths)
        }
    }
