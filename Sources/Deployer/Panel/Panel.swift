...
extension Panel {
    ...
    private func startBinaryJob(_ job: BinaryJob, request: Request) async throws -> Response {
        guard
            let idString = request.parameters.get("deploymentID"),
            ...
            throw Abort(.notFound, reason: "Deployment not found")
        }
        ...
        case .save:
            ...
            guard !store.hasBinary(for: deployment) else {
                throw Abort(.badRequest, reason: "Deployment already has a saved binary")
            }
            result = await request.application.deployer.queue.saveBinary(deployment: deployment, target: config.target)
        case .restore:
            guard deployment.canRestoreBinary else {
                throw Abort(.badRequest, reason: "Deployment does not have a saved binary or cannot be restored right now")
            }
            guard store.hasBinary(for: deployment) else {
                throw Abort(.notFound, reason: "Saved binary not found on disk")
            }
